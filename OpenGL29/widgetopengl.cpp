...
void WidgetOpenGL::initializeGL()
{
    ...
    try
    {
        ...
        glTexImage2D(GL_TEXTURE_2D, 0, GL_DEPTH_COMPONENT, SHADOW_FRAMEBUFFER_HEIGHT, SHADOW_FRAMEBUFFER_WIDTH, 0, GL_DEPTH_COMPONENT, GL_FLOAT, NULL);
...
void WidgetOpenGL::paintGL()
{
    ...
    try
    {
        ...
        // rendering cienia
        glViewport(0, 0, SHADOW_FRAMEBUFFER_HEIGHT, SHADOW_FRAMEBUFFER_WIDTH);
        glBindFramebuffer(GL_FRAMEBUFFER, FBO);
        makeShadowMatrix();
        paintScene(true);
...
void WidgetOpenGL::readTexture()
{
    ...
    tex_d.mirrored(false, true).save(".\\depthbuffer.png");
...
void WidgetOpenGL::change_resolution_shadow_map(int no_pixels)
{
    glBindTexture(GL_TEXTURE_2D, TEX_FBO_DEPTH);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_DEPTH_COMPONENT, no_pixels, no_pixels, 0, GL_DEPTH_COMPONENT, GL_FLOAT, NULL);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    ...
    SHADOW_FRAMEBUFFER_HEIGHT = no_pixels;
    SHADOW_FRAMEBUFFER_WIDTH = no_pixels;
