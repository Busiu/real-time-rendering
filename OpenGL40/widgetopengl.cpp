...
#define MODEL "grassblade2" // grassblade
#define SIDE  1000
#define SIZE  10.0
...
void WidgetOpenGL::initializeGL()
{
    ...
    try
    {
        ...
        Model model;
        model.readFile("../Modele/" MODEL ".obj", true, false, 0.1);
...
void WidgetOpenGL::paintGL()
{
    ...
    try
    {
        ...
        int attr = getUniformLocation(shaderProgram_inst, "side");
        glUniform1i(attr, SIDE);

        attr = getUniformLocation(shaderProgram_inst, "size");
        glUniform1f(attr, SIZE);

        m_matrix.setToIdentity();
        glUniformMatrix4fv(attr_mm, 1, GL_FALSE, m_matrix.data());

        glDrawArraysInstanced(GL_TRIANGLES, 0, 3*triangles_cnt, SIDE*SIDE);
