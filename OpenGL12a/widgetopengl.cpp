...
void WidgetOpenGL::initializeGL()
{
    ...
    try
    {
        ...
        qDebug() << triangles_cnt_dragon << " " << triangles_cnt_tree;
        ...
        glBufferData(GL_ARRAY_BUFFER, model_dragon.getVertDataSize(), model_dragon.getVertData(), GL_STATIC_DRAW);
        ...
        glBufferData(GL_ARRAY_BUFFER, model_tree.getVertDataSize(), model_tree.getVertData(), GL_STATIC_DRAW);
        ...
        if (attr < 0) throw QString("Nieprawidlowy parametr 'position'");
        glVertexAttribPointer(attr, 3, GL_FLOAT, GL_FALSE, model_tree.getVertDataStride()*sizeof(GLfloat), 0);
        glEnableVertexAttribArray(attr);

        // normalne
        attr = glGetAttribLocation(shaderProgram, "normal");
        if (attr < 0) throw QString("Nieprawidlowy parametr 'normal'");
        glVertexAttribPointer(attr, 3, GL_FLOAT, GL_FALSE, model_tree.getVertDataStride()*sizeof(GLfloat), (void *)(3*sizeof(GLfloat)));
        glEnableVertexAttribArray(attr);
...
void WidgetOpenGL::paintGL()
{
    ...
    try
    {
        ...
        for (int i = 0; i < 10; i++)
        {
            ...
            {
                glBindVertexArray(VAO1);
                glDrawArrays(GL_TRIANGLES, 0, 3*triangles_cnt_tree);
            }
