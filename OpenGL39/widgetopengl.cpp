...
#define MODEL         "dragon" // texcube dragon bridge starship r2d2 f1 f2
#define MODEL_TEXTURE "bridge" // dragonskingold white bridge marble starship r2d2 red f2g
#define FLOOR         "plane"
#define FLOOR_TEXTURE "wood4" // wood4 white stones2
...
void WidgetOpenGL::initializeGL()
{
    ...
    try
    {
        ...
        vertexShader   = loadShader(GL_VERTEX_SHADER,   "ssao_vertex.glsl");
        fragmentShader = loadShader(GL_FRAGMENT_SHADER, "ssao_fragment.glsl");
        shaderProgram_ssao = linkProgram(vertexShader, fragmentShader);
        ...
        Model model;
        model.readFile("../Modele/" MODEL ".obj", true, true, 0.5);
        ...
        tex_color_light = loadTexture2D("../Modele/dragonskingold.jpg", false);
        ...
        ////////////////////////////////////////////////////////////////
        // CZ 2L. Wczytanie modelu podlogi
        ////////////////////////////////////////////////////////////////

        Model model_floor;
        model_floor.readFile("../Modele/" FLOOR ".obj", true, true, 1.0);
        ...
        tex_color_floor = loadTexture2D("../Modele/" FLOOR_TEXTURE ".jpg", false);
        ...
        // view depth...
        glGenTextures(1, &tex_FBO_gBuffer_viewdepth);
        glBindTexture(GL_TEXTURE_2D, tex_FBO_gBuffer_viewdepth);
        glTexImage2D(GL_TEXTURE_2D, 0, GL_R16F, width()*devicePixelRatio(), height()*devicePixelRatio(), 0, GL_RED, GL_FLOAT, NULL);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
        ...
        glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT3, GL_TEXTURE_2D, tex_FBO_gBuffer_viewdepth, 0);
        ...
        // positions...
        glGenTextures(1, &tex_FBO_ssao_color);
        glBindTexture(GL_TEXTURE_2D, tex_FBO_ssao_color);
        glTexImage2D(GL_TEXTURE_2D, 0, GL_R16F, width()*devicePixelRatio(), height()*devicePixelRatio(), 0, GL_RED, GL_FLOAT, NULL);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);

        glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, tex_FBO_ssao_color, 0);
...
void WidgetOpenGL::paintScene()
{
    ...
    try
    {
        ...
        glUniform1f(attr_am, 0.2);

        // specular
        attr_am = getUniformLocation(shaderProgram_gbuffer, "specular");
        glUniform1f(attr_am, 0.5);

        // near i far
        int attr = getUniformLocation(shaderProgram_gbuffer, "near");
        glUniform1f(attr, NEAR_P);

        attr = getUniformLocation(shaderProgram_gbuffer, "far");
        glUniform1f(attr, FAR_P);
        ...
        m_matrix.setToIdentity();
        m_matrix.translate(0.0, -0.26, 0.0);
...
void WidgetOpenGL::paintSSAO()
{
    glClear(GL_COLOR_BUFFER_BIT);

    glUseProgram(shaderProgram_ssao);
    glBindVertexArray(VAO_screen);

    int attr = getUniformLocation(shaderProgram_ssao, "p_matrix");
    glUniformMatrix4fv(attr, 1, GL_FALSE, p_matrix.data());

    attr = getUniformLocation(shaderProgram_ssao, "v_matrix");
    glUniformMatrix4fv(attr, 1, GL_FALSE, v_matrix.data());

    attr = getUniformLocation(shaderProgram_ssao, "near");
    glUniform1f(attr, NEAR_P);

    attr = getUniformLocation(shaderProgram_ssao, "far");
    glUniform1f(attr, FAR_P);

    attr = getUniformLocation(shaderProgram_ssao, "ssao_radius");
    glUniform1f(attr, ssao_radius);

    glActiveTexture(GL_TEXTURE0);
    glBindTexture(GL_TEXTURE_2D, tex_FBO_gBuffer_position);
    attr = getUniformLocation(shaderProgram_ssao, "gPosition");
    glUniform1i(attr, 0);

    glActiveTexture(GL_TEXTURE1);
    glBindTexture(GL_TEXTURE_2D, tex_FBO_gBuffer_viewdepth);
    attr = getUniformLocation(shaderProgram_ssao, "gDepth");
    glUniform1i(attr, 1);

    glActiveTexture(GL_TEXTURE2);
    glBindTexture(GL_TEXTURE_2D, tex_FBO_gBuffer_normal);
    attr = getUniformLocation(shaderProgram_ssao, "gNormal");
    glUniform1i(attr, 2);


    for (int i = 0; i < ssao_kernel_size; i++)
    {
        QString a = QString("ssao_kernel[%1]").arg(i);
        attr = getUniformLocation(shaderProgram_ssao, a.toLocal8Bit().constData());
        glUniform3f(attr, ssao_kernel[i].x(), ssao_kernel[i].y(), ssao_kernel[i].z());
    }
...
void WidgetOpenGL::paintAndShade()
{
    ...
    glBindTexture(GL_TEXTURE_2D, tex_FBO_ssao_color);
    t = getUniformLocation(shaderProgram_shading, "gSSAO");
    glUniform1i(t, 3);

    t = getUniformLocation(shaderProgram_shading, "slider");
    glUniform1i(t, slider);
...
void WidgetOpenGL::resizeGL(int w, int h)
{
    ...
    // macierz perspektywy...
    p_matrix.perspective(45, r, NEAR_P, FAR_P);
    p_matrix.translate(0, 0, -5);

    // przeskalowanie tekstur framebuforow...
    glBindTexture(GL_TEXTURE_2D, tex_FBO_gBuffer_position);
    ...
    glBindTexture(GL_TEXTURE_2D, tex_FBO_gBuffer_viewdepth);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_R16F, w*devicePixelRatio(), h*devicePixelRatio(), 0, GL_RED, GL_FLOAT, NULL);
    glBindTexture(GL_TEXTURE_2D, tex_FBO_gBuffer_depth);
...
void WidgetOpenGL::generateSSAOKernel()
{
    for (int i = 0; i < ssao_kernel_size; i++)
    {
        ssao_kernel[i].setX(f_rand(-1, 1));
        ssao_kernel[i].setY(f_rand(-1, 1));
        ssao_kernel[i].setZ(f_rand(-1, 1));
        ssao_kernel[i].normalize();
        float scale = f_rand(0.2, 1.0);
        ssao_kernel[i] *= scale*scale;
    }
}
...
void WidgetOpenGL::set_ssao_radius(float _radius)
{
    ssao_radius = _radius;
