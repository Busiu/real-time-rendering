...
void WidgetOpenGL::paintGL()
{
    ...
    try
    {
        ...
        // material...
        // Uzylem dwoch roznych materialow (emerald oraz obsydian), aby zobaczyc jak beda wygladaly nasze smoki.
        // Model oswietlenia Phonga nie jest w stanie oczywiscie wygenerowac wszystkich efektow. Dobrze sobie radzi
        // z symulowaniem roznych materialow, lecz niestety ciezko by bylo za pomoca niego zasymulowac np. odbicia
        // anizotropowe.
        std::vector<Material> materials;

        materials.push_back(Material(0.135f,    0.2225f,    0.1575f,    0.2f,
                                     0.54f, 	0.89f,      0.63f,      1.0f,
                                     0.316228f, 0.316228f, 	0.316228f,  1.0f,
                                     0.1f*128.0f));
        materials.push_back(Material(0.05375f, 	0.05f,      0.06625f,   0.2f,
        ...
        for (int i = 0; i < 10; i++)
        {
            attr_material = glGetUniformLocation(shaderProgram, "material.ambient");
            if (attr_material < 0) throw QString("Nieprawidlowy parametr 'material.ambient'");
            glUniform3fv(attr_material, 1, materials[i%2].ambient);
            attr_material = glGetUniformLocation(shaderProgram, "material.diffuse");
            if (attr_material < 0) throw QString("Nieprawidlowy parametr 'material.diffuse'");
            glUniform3fv(attr_material, 1, materials[i%2].diffuse);
            attr_material = glGetUniformLocation(shaderProgram, "material.specular");
            if (attr_material < 0) throw QString("Nieprawidlowy parametr 'material.specular'");
            glUniform3fv(attr_material, 1, materials[i%2].specular);
            attr_material = glGetUniformLocation(shaderProgram, "material.shininess");
            if (attr_material < 0) throw QString("Nieprawidlowy parametr 'material.shininess'");
            glUniform1f(attr_material, materials[i%2].shininess);
