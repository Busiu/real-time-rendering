...
#define TEXTURE "wood"    //  wood bricks stones stones2 stones3 bricks2
...
GLuint WidgetOpenGL::getUniformLocation(GLuint program, const GLchar *uniform)
{
    ...
    if (a < 0) throw QString("Nieprawidlowy parametr '%1' (program %2), error %3").arg(uniform).arg(program).arg(a);
    return a;
}
...
void WidgetOpenGL::paintGL()
{
    ...
    try
    {
        ...
        // paralax
        int attr_paralax = getUniformLocation(shaderProgram, "minLayers");
        glUniform1f(attr_paralax, min_layers);
        attr_paralax = getUniformLocation(shaderProgram, "maxLayers");
        glUniform1f(attr_paralax, max_layers);
        attr_paralax = getUniformLocation(shaderProgram, "heightScale");
        glUniform1f(attr_paralax, height_scale);
...
void WidgetOpenGL::change_paralax_params(float _min_layers, float _max_layers, float _height_scale)
{
    min_layers = _min_layers;
