...
void WidgetOpenGL::initializeGL()
{
    ...
    try
    {
        ...
        tex_color_light = loadTexture2D("../Modele/test_color.jpg", false);
        ...
        // id...
        // !!!
        glGenTextures(1, &tex_FBO_gBuffer_id);
        glBindTexture(GL_TEXTURE_2D, tex_FBO_gBuffer_id);
        glTexImage2D(GL_TEXTURE_2D, 0, GL_R32F, width()*devicePixelRatio(), height()*devicePixelRatio(), 0, GL_RED, GL_FLOAT, NULL);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
        ...
        glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT3, GL_TEXTURE_2D, tex_FBO_gBuffer_id, 0);
...
void WidgetOpenGL::paintScene()
{
    ...
    try
    {
        ...
        for (int i = 0; i < 10; i++)
        {
            // !!!
            int object_id = i + 1;
            int attr_id = getUniformLocation(shaderProgram_gbuffer, "object_id");
            glUniform1i(attr_id, object_id);

            int attr_is = getUniformLocation(shaderProgram_gbuffer, "is_selected");
            glUniform1i(attr_is, object_id == selected_object);
...
void WidgetOpenGL::paintAndShade()
{
    ...
    glBindTexture(GL_TEXTURE_2D, tex_FBO_gBuffer_id);
    t = getUniformLocation(shaderProgram_shading, "gId");
    glUniform1i(t, 3);
    ...
    int attr_s = getUniformLocation(shaderProgram_shading, "slider");
...
void WidgetOpenGL::mousePressEvent(QMouseEvent *mouseEvent)
{
    // int w = width();
    int h = height();
    int x = mouseEvent->pos().x();
    int y = h - mouseEvent->pos().y();
...
//    selected_object = ids[w*y + x];
//    delete [] ids;


    // dla OpenGL >= 4.5
    float id;
    glGetTextureSubImage(tex_FBO_gBuffer_id, 0, x, y, 0, 1, 1, 1, GL_RED, GL_FLOAT, sizeof(id), &id);
    selected_object = id;
...
void WidgetOpenGL::resizeGL(int w, int h)
{
    ...
    glBindTexture(GL_TEXTURE_2D, tex_FBO_gBuffer_id);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_R32F, w*devicePixelRatio(), h*devicePixelRatio(), 0, GL_RED, GL_FLOAT, NULL);
    glBindTexture(GL_TEXTURE_2D, tex_FBO_gBuffer_depth);
