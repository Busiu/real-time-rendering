...
//        texarray = loadTexturesFromRaw("../Modele/Vol/skull.raw", dataInfo);
        ...
        mainWindow->setSplitter(dataInfo.slices);
...
void WidgetOpenGL::paintGL()
{
    ...
    try
    {
        ...
        float zoom_s = dataInfo.spacing_z*dataInfo.slices*gap;

        for (int i = 0; i < dataInfo.slices; i++)
        {
            if (i > slider) continue;

            m_matrix.setToIdentity();
            m_matrix.scale(zoom_w*zoom_factor, zoom_s*zoom_factor, zoom_h*zoom_factor);
            m_matrix.translate(0, i/(dataInfo.slices - 1.0) - 0.5, 0);
...
void WidgetOpenGL::set_gap(float gap)
{
    this->gap = gap;
    repaint();
}
