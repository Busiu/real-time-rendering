...
vec3 texChess(vec3 p)
{
    const vec3 colorBlack = vec3(0.0, 0.0, 0.0);
    const vec3 colorWhite = vec3(1.0, 1.0, 1.0);

    const int density = 8;



    float xCoord = -((p.x - 0.8001) / 1.6002);
    float yCoord = -((p.y - 0.8001) / 1.6002);
    float zCoord = -((p.z - 0.8001) / 1.6002);

    int row = int(density * xCoord);
    int col = int(density * yCoord);
    int deepness = int(density * zCoord);

    int tileNumber = row + col + deepness;
