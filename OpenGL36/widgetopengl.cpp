...
void WidgetOpenGL::initializeGL()
{
    ...
    try
    {
        ...
        Model model_shadow;
        model_shadow.readFile("../Modele/" MODEL_SHADOW ".obj", true, true, 0.8);
...
void WidgetOpenGL::paintFilter(GLuint textureIn, boolean horizontal)
{
    ...
    //qDebug() << "w = " << w[0];
    int attr_w = getUniformLocation(shaderProgram_Filter, "w");
    glUniform1fv(attr_w, 20, w);
    //qDebug() << "w_length = " << w_length;
    int attr_w_length = getUniformLocation(shaderProgram_Filter, "wLength");
    glUniform1i(attr_w_length, w_length);
...
void WidgetOpenGL::change_gaussian_radius(int radius)
{
    w_length = radius;
    passGaussianVector();
...
void WidgetOpenGL::initializeGaussianTriangle()
{
    initializePascalTriangle();
    normalizeToGaussianTriangle();
    passGaussianVector();
}

void WidgetOpenGL::initializePascalTriangle()
{
    pascalTriangle[0][0] = 1;

    for (int i = 1; i < 2 * w_max_length; i++)
    {
        for (int j = 0; j <= i; j++)
        {
            if (j == 0 || j == i)
...
void WidgetOpenGL::normalizeToGaussianTriangle()
{
    for (int i = 0; i < w_max_length; i++)
    {
        float power = pow(2, 2 * i);
        for (int j = 0; j <= i; j++)
        {
            gaussianTriangle[i][j] = pascalTriangle[2 * i][j + i] / power;
        }
    }
}

void WidgetOpenGL::passGaussianVector()
{
    for (int i = 0; i < w_length; i++)
    {
        w[i] = gaussianTriangle[w_length - 1][i];
        //qDebug() << w[i];
    }
    for (int i = w_length; i < w_max_length; i++)
    {
        w[i] = 0;
        //qDebug() << w[i];
    }
